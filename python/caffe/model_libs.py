...
def CreateAnnotatedDataLayer(source, batch_size=32, backend=P.Data.LMDB,
    ...
    if train:
        kwargs = {
                'include': dict(phase=caffe_pb2.Phase.Value('TRAIN')),#python/caffe/proto/caffe_pb2.py
    ...
    return L.AnnotatedData(name="data", annotated_data_param=annotated_data_param,#caffe/src/caffe/layers/annotated_data_layer.cpp
...
#reduced 通过conv减少channel数量
#dilated 增大感受野 see paper of Multi-Scale Context Aggregation by Dilated Convolutions
#need_fc fully convolution 替代全连接的一个方法，该方法参数少
def VGGNetBody(net, from_layer, need_fc=True, fully_conv=False, reduced=False,
    ...
    kwargs = {
            'param': [dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)],
            'weight_filler': dict(type='xavier'),#weight，bias初始化
            'bias_filler': dict(type='constant', value=0)}
