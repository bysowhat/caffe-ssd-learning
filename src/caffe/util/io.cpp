...
bool ReadXMLToAnnotatedDatum(const string& labelfile, const int img_height,
  ...
  int instance_id = 0;
  BOOST_FOREACH(ptree::value_type &v1, pt.get_child("annotation")) {//循环annotation的所有子节点
    ptree pt1 = v1.second;
    if (v1.first == "object") {//"object"一个物体一个object
      Annotation* anno = NULL;
      bool difficult = false;
      ptree object = v1.second;
      BOOST_FOREACH(ptree::value_type &v2, object.get_child("")) {
        ptree pt2 = v2.second;
        if (v2.first == "name") {//name
          string name = pt2.data();
          if (name_to_label.find(name) == name_to_label.end()) {//label
        ...
        } else if (v2.first == "difficult") {//difficult
          difficult = pt2.data() == "1";
        } else if (v2.first == "bndbox") {//bndbox
          ...
          bbox->set_xmin(static_cast<float>(xmin) / width);//boundingbox数值归一化
