...
void AnnotatedDataLayer<Dtype>::DataLayerSetUp(
  ...
  const int batch_size = this->layer_param_.data_param().batch_size();//设置batch_size
  const AnnotatedDataParameter& anno_data_param =//为数据的增广而设置的采样batch
  ...
  if (this->output_labels_) {
    ...
    if (has_anno_type_) {//是否有bbox信息，而不只是一个分类id
      ...
      if (anno_type_ == AnnotatedDatum_AnnotationType_BBOX) {
        ...
        // instance_id.
    	//item_id代表第几张图片，group_label代表的是Bbox的类别，instance_id代表Bbox的个数
    	//label_shape:[1,1,bboxes数量,8]
        ...
        label_shape[2] = std::max(num_bboxes, 1);//bboxes数量
        label_shape[3] = 8;//[item_id, group_label, instance_id, xmin, ymin, xmax, ymax, diff]
      } else {
